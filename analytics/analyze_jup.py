...
def print_trace(i, visited, acc, offset=0):
    for old in visited:
    ...
    for j in matching_indices[i]:
        if j > i:
            print_trace(j, visited | set((i,)), acc, offset + 2)

visited = set()
paths = []
for i in sorted(matching_indices.keys()):
    paths.append([])
    print_trace(i, visited, paths)
...
if True:
    ...
    for k, pt in entity_stats.items():
        ...
        for tag, stats in vs:
            ...
            if stats['count'] > 5 and diff > 0 and printed < 10 and len(tag) < 20:
                printed += 1
